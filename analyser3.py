...
def create_automation_script():
    ...
    # Use a list to build the script lines for clarity and correctness
    script_lines = [
        "#!/bin/bash",
    ...
    # Assuming Part A files are in partA/ directory
    if os.path.exists(PATH_PART_A):
        part_a_tcl_files = sorted([f for f in os.listdir(PATH_PART_A) if f.endswith('.tcl')])
        if part_a_tcl_files:
            for tcl_file in part_a_tcl_files:
                full_path = os.path.join(PATH_PART_A, tcl_file)
                script_lines.append(f'echo "   -> Running {full_path}"')
                script_lines.append(f"ns {full_path}")
        else:
            script_lines.append(f'echo "   -> No Part A TCL files found in {PATH_PART_A}."')
    else:
        script_lines.append(f'echo "   -> Directory not found: {PATH_PART_A}"')


    # --- Part B Simulations ---
    script_lines.append('')
    script_lines.append('echo ">>> Running Part B Simulations (RED)..."')
    # Assuming Part B files are in partB/ directory
    if os.path.exists(PATH_PART_B):
        part_b_tcl_files = sorted([f for f in os.listdir(PATH_PART_B) if f.endswith('.tcl')])
        if part_b_tcl_files:
            for tcl_file in part_b_tcl_files:
                full_path = os.path.join(PATH_PART_B, tcl_file)
                script_lines.append(f'echo "   -> Running {full_path}"')
                script_lines.append(f"ns {full_path}")
        else:
            script_lines.append(f'echo "   -> No Part B TCL files found in {PATH_PART_B}."')
    else:
        script_lines.append(f'echo "   -> Directory not found: {PATH_PART_B}"')
    ...
    if os.path.exists(part_c_tcl_file):
        ...
        script_lines.append(f'echo "   -> Part C TCL file not found at {part_c_tcl_file}."')
    ...
    # --- Join all lines with REAL newlines and write the script ---
    script_content = "\n".join(script_lines)
...
# --- END: CORRECTED version of the function ---
