...
# --- FINAL, COMPLETE version of the 'Smart Scan' function ---

def create_automation_script(python_cmd='python3'): # Changed default back to python3 for better compatibility
    """
    Dynamically scans for ALL TCL simulation files, including sensitivity tests,
    and creates a shell script to automate the entire workflow.
    """
    print("\n" + "="*70)
    print("PACKAGING: Dynamically creating Automation Script 'run_all.sh'")
    print("="*70)
    ...
    # --- Part A Simulations ---
    script_lines.append('')
    script_lines.append('echo ">>> Running Part A Simulations (DropTail)..."')
    path_a = PATH_PART_A
    if os.path.exists(path_a):
        tcl_files = sorted([f for f in os.listdir(path_a) if f.endswith('.tcl')])
        if tcl_files:
            for tcl_file in tcl_files:
                full_path = os.path.join(path_a, tcl_file)
                script_lines.append(f'echo "   -> Running {full_path}"')
                script_lines.append(f"ns {full_path}")
        else: script_lines.append(f'echo "   -> No TCL files found in {path_a}."')
    else: script_lines.append(f'echo "   -> Directory not found: {path_a}"')

    # --- Part B Simulations ---
    script_lines.append('')
    script_lines.append('echo ">>> Running Part B Simulations (RED)..."')
    path_b = PATH_PART_B
    if os.path.exists(path_b):
        # Scan for regular Part B files, excluding the sensitivity subdirectory
        tcl_files = sorted([f for f in os.listdir(path_b) if f.endswith('.tcl') and not os.path.isdir(os.path.join(path_b, f))])
        if tcl_files:
            for tcl_file in tcl_files:
                full_path = os.path.join(path_b, tcl_file)
                script_lines.append(f'echo "   -> Running {full_path}"')
                script_lines.append(f"ns {full_path}")
        else: script_lines.append(f'echo "   -> No regular TCL files found in {path_b}."')
    else: script_lines.append(f'echo "   -> Directory not found: {path_b}"')
    
    # --- Part B SENSITIVITY Simulations ---
    script_lines.append('')
    script_lines.append('echo ">>> Running Part B Sensitivity Simulations..."')
    path_b_sens = os.path.join(PATH_PART_B, 'sensitivity')
    if os.path.exists(path_b_sens):
        tcl_files = sorted([f for f in os.listdir(path_b_sens) if f.endswith('.tcl')])
        if tcl_files:
            for tcl_file in tcl_files:
                full_path = os.path.join(path_b_sens, tcl_file)
                script_lines.append(f'echo "   -> Running {full_path}"')
                script_lines.append(f"ns {full_path}")
        else: script_lines.append(f'echo "   -> No TCL files found in {path_b_sens}."')
    else: script_lines.append(f'echo "   -> Directory not found: {path_b_sens}"')


    # --- Part C Simulations ---
    script_lines.append('')
    script_lines.append('echo ">>> Running Part C Simulations (Reproducibility)..."')
    part_c_tcl_file = os.path.join(PATH_PART_C, 'cubicCodePartC.tcl')
    if os.path.exists(part_c_tcl_file):
        script_lines.append("for i in {1..5}")
        script_lines.append("do")
        script_lines.append(f'    echo "   -> Running {part_c_tcl_file} with seed $i"')
        script_lines.append(f"    ns {part_c_tcl_file} $i")
        script_lines.append("done")
    else: script_lines.append(f'echo "   -> Part C TCL file not found at {part_c_tcl_file}."')
    ...
    try:
        ...
        if os.name == 'posix': os.chmod(script_filename, 0o755)
        print(f"Successfully created executable script '{script_filename}'.")
        print("You can run it from your terminal with: ./run_all.sh")
